...
#include "../include/layer/details/maxpooling.hpp"

using namespace magic_infer;


void MaxPooling(const vector<shared_ptr<Tensor<float>>> &inputs, vector<shared_ptr<Tensor<float>>> &outputs,
    int stride_w, int stride_h, int kernel_h, int kernel_w) 
{
    ...
    for (uint32_t i = 0; i < batch; ++i) {
        ...
        const uint32_t output_h = uint32_t(floor((input_h - kernel_h) / stride_h + 1));
        const uint32_t output_w = uint32_t(floor((input_w - kernel_w) / stride_w + 1));
