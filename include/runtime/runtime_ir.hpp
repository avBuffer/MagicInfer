#ifndef MAGIC_RUNTIME_RUNTIME_IR_HPP_
#define MAGIC_RUNTIME_RUNTIME_IR_HPP_
...
public:
     ...
     * @param operators MagicInfer计算图中的计算节点
     */
...
private:
    ...
    /**
     * 初始化magic infer计算图节点中的输入操作数
     * @param inputs pnnx中的输入操作数
     * @param runtime_operator 计算图节点
     */
    static void InitInputOperators(const std::vector<pnnx::Operand *> &inputs, const std::shared_ptr<RuntimeOperator> &runtime_operator);

    /**
     * 初始化magic infer计算图节点中的输出操作数
     * @param outputs pnnx中的输出操作数
     * @param runtime_operator 计算图节点
     */
    static void InitOutputOperators(const std::vector<pnnx::Operand *> &outputs, const std::shared_ptr<RuntimeOperator> &runtime_operator);

    /**
     * 初始化magic infer计算图中的节点属性
    ...
    /**
     * 初始化magic infer计算图中的输入操作数
     * @param src 上一个节点的输出操作数
     * @param dest 下一个节点的输入操作数
     */
    static void SetOpInputData(const std::vector<std::shared_ptr<Tensor<float>>> &src, const std::vector<std::shared_ptr<Tensor<float>>> &dest);
     ...
     * @param operator_queue 计MAGIC_RUNTIME_RUNTIME_PARAMETER_HPP_算节点的计算序列
     * @param layer_output_data 当前节点的输出，赋予到下一层计算节点的输入张量中
     */
    static void ProbeNextLayer(const std::shared_ptr<RuntimeOperator> &current_op, std::deque<std::shared_ptr<RuntimeOperator>> &operator_queue,
...
#endif //MAGIC_RUNTIME_RUNTIME_IR_HPP_
