...
/* natural logarithm computed for 4 simultaneous float
   return NaN for x <= 0
*/
static MAGIC_FORCEINLINE v4sf log_ps(v4sf x) 
{
#ifdef USE_SSE2
    v4si emm0;
#else
...
static MAGIC_FORCEINLINE v4sf exp_ps(v4sf x) 
{
...
/* evaluation of 4 sines at onces, using only SSE1+MMX intrinsics so
   it runs also on old athlons XPs and the pentium III of your grand mother.

   The code is the exact rewriting of the cephes sinf function.
   Precision is excellent as long as x < 8192 (I did not bother to
   take into account the special handling they have for greater values
   -- it does not return garbage for arguments over 8192, though, but the extra precision is missing).
...
*/
static MAGIC_FORCEINLINE v4sf sin_ps(v4sf x) 
{ // any x
...
static MAGIC_FORCEINLINE v4sf cos_ps(v4sf x) 
{ // any x
...
static MAGIC_FORCEINLINE void sincos_ps(v4sf x, v4sf *s, v4sf *c) 
{
...
static MAGIC_FORCEINLINE __m128 tan_ps(__m128 x) 
{
    __m128 ysin, ycos;
...
static MAGIC_FORCEINLINE __m128 pow_ps(__m128 a, __m128 b) 
{
    // pow(x, m) = exp(m * log(x))
