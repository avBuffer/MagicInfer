...
void window_cal(const int w, const int k, const int s, const int p, int win, vector<int>* conv_idx, 
    vector<int>* tile_idx, vector<int>* ac_before_tile, vector<int>* tile_size, vector<int>* in_win);

void print_var(vector<int>* var, string *s);
void print_var(vector<float>* var, string *s);

void init_in(vector<float>* img, int H, int W);
void init_out(vector<float>* img, int H, int W);
void init_kernel(vector<float>* img, int out_c, int in_c, int k);

int pad(int w, int v);
void init(float* df, int h, int w);


extern void launch_im2col(float *in, int c, int h_size, int h_size_w, int h_size_h, int w_size,
    int h_size_pad, int w_size_pad, int h, int w, int k, int s, float *out);
extern void launch_kernel2col(float *in, int out_c, int in_c, int out_c_pad, int in_c_pad, int k, float *out);

extern void launch_sgemm(int m, int n, int k, float *a, float *b, float *c);

void launchResizeKernel(float *mean, float *std, const char *in_image, int in_height, int in_width,
    float *out_image, int out_height, int out_width, int letter_height, int letter_width, float ratio,
    int offset_row, int offset_col);

void launchResizeKernel(const char *in_image, int in_height, int in_width, float *out_image, int out_height, int out_width,
    int letter_height, int letter_width, float ratio, int offset_row, int offset_col);
...
void window_cal(const int w, const int k, const int s, const int p, int win,
    vector<int>* conv_idx, vector<int>* tile_idx, vector<int>* ac_before_tile,
    vector<int>* tile_size, vector<int>* in_win)
{
    ...
    for (int i = 0; i < conv_cnts; ++i) {
        ...
        if ( i == 0) {
            ...
            cout << "window_cal line= " << __LINE__ << endl;
...
int pad(int w, int v) { return (w + v - 1) &(-v); }
...
int main()
{
    ...
    vector<float> h_out;
    cout << "h_size = " << h_size << " w_size = " << w_size << endl;
    ...
    launch_im2col(d_in, c, h_size, h_size_w, h_size_h, w_size, h_size_pad, w_size_pad, h, w, k, s, d_out);
    cudaMemcpy(&h_out[0], d_out, h_out.size() * sizeof(float), cudaMemcpyDeviceToHost);

    cout << "h_out.size() = " << h_out.size() << "\th_size = " << h_size << " h_size_h = " << h_size_h << " h_size_w = " << h_size_w << endl;
    ofstream ofs;
    ...
    float ratio = min((float)out_length / (float)in_height, (float)out_length / (float)in_width);
    ...
    launchResizeKernel(d_mean, d_std, d_in_image, in_height, in_width, d_out_image, 
        out_height, out_width, letter_height, letter_width, ratio, offset_row, offset_col);
    ...
    cout << "in_height = " << in_height << " in_width = " << in_width << endl;
    cout << "ratio = " << ratio << " letter_height = " << letter_height << " letter_width = " << letter_width << endl;
    cout << "out_height = " << out_height << " out_width = " << out_width << endl;
