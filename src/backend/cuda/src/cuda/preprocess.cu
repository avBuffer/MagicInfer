...
__global__ void setValueKernel(float *values, int numValue, float value)
{
    ...
    if (tid < numValue) values[tid] = value;
}
...
__global__ void resizKernel(float *mean, float *std, const char *in_image, int in_height, int in_width,
...
__global__ void resizKernel(const char *in_image, int in_height, int in_width,
    float *out_image, int out_height, int out_width, int letter_height, int letter_width, float ratio,
    int offset_row, int offset_col)
{
    int tid = blockIdx.x * blockDim.x + threadIdx.x;
...
void launchResizeKernel(const char *in_image, int in_height, int in_width,
    float *out_image, int out_height, int out_width, int letter_height, int letter_width, float ratio,
    int offset_row, int offset_col)
{
    ...
    resizKernel<<<grid, kThreadsPerBlock>>>(in_image, in_height, in_width, 
        out_image, out_height, out_width, letter_height, letter_width, ratio, offset_row, offset_col);
    ...
    int offset_row, int offset_col)
{
    ...
    resizKernel<<<grid, kThreadsPerBlock>>>(mean, std, in_image, in_height, in_width, 
        out_image, out_height, out_width, letter_height, letter_width, ratio, offset_row, offset_col);
    cudaDeviceSynchronize();
}
