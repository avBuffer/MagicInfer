...
Attribute::Attribute(const initializer_list<int>& _shape, const vector<float>& t)
{
    type = 1;
    shape = _shape;
...
Parameter Parameter::parse_from_string(const string& value)
{
    Parameter p;
    p.type = 0;
    if (value == "None" || value == "()" || value == "[]") return p;
    ...
    if (value[0] == '(' || value[0] == '[') {
        // list
        string lc = value.substr(1, value.size() - 2);
        istringstream lcss(lc);
        ...
        while (!lcss.eof()) {
            ...
            } else if (elem.find('.') != string::npos || elem.find('e') != string::npos) {
                // float
                p.type = 6;
                p.af.push_back(stof(elem));
            
            } else {
                // integer
                p.type = 5;
                p.ai.push_back(stoi(elem));
            }
        }
        return p;
    }
    ...
    if (value.find('.') != string::npos || value.find('e') != string::npos) {
        // float
        p.type = 3;
        p.f = stof(value);
        return p;
...
static void load_parameter(Operator* op, const string& key, const string& value)
{
    op->params[key] = Parameter::parse_from_string(value);
}


static void load_input_key(Operator* op, const string& key, const string& value)
{
...
static void load_shape(Operator* op, const string& key, const string& value)
{
    Operand* operand = 0;
    ...
    // type
    string typestr = value.substr(value.find_last_of(')') + 1);
    operand->type = string_to_type(typestr.c_str());

    // shape
    string lc = value.substr(1, value.find_last_of(')') - 1);
    istringstream lcss(lc);
...
static void load_attribute(Operator* op, const string& key, const string& value, StoreZipReader& szr)
{
    ...
    string filename = op->name + "." + key;
    size_t filesize = szr.get_file_size(filename);
    if (filesize == 0) return; // no such file
...
int Graph::load(const string& parampath, const string& binpath)
{
    ifstream is(parampath, ios::in | ios::binary);
    if (!is.good()) {
...
int Graph::save(const string& parampath, const string& binpath)
{
    ...
    for (const Operator* op : ops) {
        ...
        for (const auto& it : op->params) {
            ...
            if (param.type == 0) fprintf(paramfp, "None");

            if (param.type == 1)  {
                if (param.b) fprintf(paramfp, "True");
                else fprintf(paramfp, "False");
            }
            
            if (param.type == 2) fprintf(paramfp, "%d", param.i);
            if (param.type == 3) fprintf(paramfp, "%e", param.f);
            if (param.type == 4) fprintf(paramfp, "%s", param.s.c_str());
        ...
        for (const auto& it : op->attrs)
        {
            ...
            string filename = op->name + "." + it.first;
        ...
        for (const Operand* oprand : op->inputs) {
            ...
            for (int i = 0; i < (int)oprand->shape.size() - 1; i++) {
                if (oprand->shape[i] == -1) fprintf(paramfp, "?,");
                else fprintf(paramfp, "%d,", oprand->shape[i]);
            }

            if (oprand->shape.size() > 0) {
                if (oprand->shape[oprand->shape.size() - 1] == -1) fprintf(paramfp, "?");
                else fprintf(paramfp, "%d", oprand->shape[oprand->shape.size() - 1]);
            }
...
static string sanitize_identifier(const string& s)
{
    string ss = s;
...
static string expand_expression(const Operator* op)
{
    string expr = op->params.at("expr").s;
    ...
    for (int i = (int)tokens.size() - 1; i >= 0; i--) {
        ...
        if (t == "size") {
            ...
            exprstack.push(r);
                   ...
                   t == "floor" || t == "log"   || t == "neg"  || t == "reciprocal" || t == "rsqrt"  || t == "sign"  || 
                   t == "sin"   || t == "sinh"  || t == "sqrt" || t == "square"     || t == "tan"    || t == "tanh"  || 
                   t == "trunc") {
            string unaryop;
            ...
            if (!elements.empty()) r += elements[elements.size() - 1];
            r += "]";
            exprstack.push(r);
        
        } else if (t[0] == '@'){
            int input_index = stoi(t.substr(1));
            string varid = string("v_") + sanitize_identifier(op->inputs[input_index]->name);
    ...
    string r = exprstack.top();
    exprstack.pop();
    return r;
}


static string make_slice_expression(const Operator* op)
{
    ...
    vector<int> dims;
    if (op->params.find("dims") != op->params.end()) dims = op->params.at("dims").ai;
    else dims.push_back(op->params.at("dim").i);
    ...
    for (int i = 0; i < ndim; i++) {
        ...
        if (op->params.find("starts") != op->params.end()) {
            vector<int> starts = op->params.at("starts").ai;
            int start = starts[i];
            if (start != 0) r += to_string(start);
            ...
            for (size_t j = 0; j < op->inputnames.size(); j++) {
                if (op->inputnames[j] == "start") {
                    r += string("v_") + sanitize_identifier(op->inputs[j]->name);
        ...
        if (op->params.find("ends") != op->params.end()) {
            vector<int> ends = op->params.at("ends").ai;
            int end = ends[i];
            if (end != INT_MAX) r += to_string(end);
        ...
        if (op->params.find("steps") != op->params.end()) {
            vector<int> steps = op->params.at("steps").ai;
            int step = steps[i];
            if (step != 1) {
                r += ':';
                r += to_string(step);
...
static string make_index_expression(const Operator* op)
{
    fprintf(stderr, "make_index_expression %s\n", op->name.c_str());
    string index_expr = op->params.at("expr").s;
    // strip out-most [ ] pair
    ...
    if (leading_none) index_expr = "...," + index_expr;
    return index_expr;
}


int Graph::python(const string& pypath, const string& pnnxbinpath)
{
    ...
    // module
    {
        for (const Operator* op : ops) {
            ...
            if (op->type == "nn.quantized.Conv2d" || op->type == "nn.quantized.Linear") param_count -= 2; // ignore scale and zero_point
            ...
            for (const auto& it : op->params) {
                ...
                const Parameter& param = it.second;
                if (param.type == 0) fprintf(pyfp, "None");

                if (param.type == 1) {
                    if (param.b) fprintf(pyfp, "True");
                    else fprintf(pyfp, "False");
                }

                if (param.type == 2) fprintf(pyfp, "%d", param.i);
                if (param.type == 3) fprintf(pyfp, "%f", param.f);
       
                if (param.type == 4) {
                    if (param.s.substr(0, 6) == "torch.") fprintf(pyfp, "%s", param.s.c_str());                 
                    else fprintf(pyfp, "\'%s\'", param.s.c_str());
                }
                ...
                if (param.type == 7) {
                    fprintf(pyfp, "(");
                    for (size_t i = 0; i < param.as.size(); i++) {
                        if (param.as[i].substr(0, 6) == "torch.") fprintf(pyfp, "%s", param.as[i].c_str());
                        else fprintf(pyfp, "\'%s\'", param.as[i].c_str());
    ...
    // load weights
    {
        ...
        for (const Operator* op : ops) {
            ...
            for (const auto& it : op->attrs) {
                ...
                if (attr.type == 1 || attr.type == 2 || attr.type == 3) fprintf(pyfp, "), '%s')\n", type_to_numpy_string(attr.type));
                else fprintf(pyfp, "), '%s', requires_grad=False)\n", type_to_numpy_string(attr.type));
            }
        }
        ...
        for (const Operator* op : ops) {
            ...
            const string& key = op->attrs.begin()->first;
            ...
            bool is_running_mean_var = false;
            {
                ...
                if (r->consumers.size() == 1) {
                    ...
                    if (op2->type == "F.batch_norm" || op2->type == "F.instance_norm") {
                        if (r == op2->inputs[1] || r == op2->inputs[2]) is_running_mean_var = true;
                    }
                }
            }

            if (is_running_mean_var) fprintf(pyfp, "        self.%s_%s = self.load_pnnx_bin_as_tensor(archive, '%s.%s', (", sanitize_identifier(op->name).c_str(), sanitize_identifier(key).c_str(), op->name.c_str(), key.c_str());
            else fprintf(pyfp, "        self.%s_%s = self.load_pnnx_bin_as_parameter(archive, '%s.%s', (", sanitize_identifier(op->name).c_str(), sanitize_identifier(key).c_str(), op->name.c_str(), key.c_str());
    ...
    // forward body
    {
        for (const Operator* op : ops) {
            ...
            if (op->type == "pnnx.Expression") {
                ...
                string expanded_expr = expand_expression(op);
                fprintf(pyfp, " = %s\n", expanded_expr.c_str());
                ...
                // slice expr
                string slice_expr = make_slice_expression(op);
                ...
                if (op->inputs.size() == 2) {
                    ...
                    const vector<int>& shape = op->params.at("shape").ai;
                ...
                if (op->inputs.size() == 2) {
                    ...
                    const vector<int>& sizes = op->params.at("sizes").ai;
                ...
                if (op->outputs.size() == 1) fprintf(pyfp, "v_%s, _", sanitize_identifier(op->outputs[0]->name).c_str());
                else fprintf(pyfp, "v_%s, (v_%s, v_%s)", sanitize_identifier(op->outputs[0]->name).c_str(), sanitize_identifier(op->outputs[1]->name).c_str(), sanitize_identifier(op->outputs[2]->name).c_str());
            ...
            } else if (op->type.find("::") != string::npos || op->type.find(".") != string::npos) {
                // direct
                for (size_t i = 0; i < op->outputs.size(); i++) {
                    fprintf(pyfp, "v_%s", sanitize_identifier(op->outputs[i]->name).c_str());
                    if (i + 1 != op->outputs.size()) fprintf(pyfp, ", ");
                ...
                int i = 0;
                for (const auto& it : op->params) {
                    if (op->type.substr(0, 7) == "Tensor." && i == 0) fprintf(pyfp, "%s=", it.first.c_str());
                    else if (op->inputs.empty() && i == 0) fprintf(pyfp, "%s=", it.first.c_str());
                    else fprintf(pyfp, ", %s=", it.first.c_str());
                    i++;
    ...
    // export torchscript
    {
        ...
        for (const Operator* op : ops) {
            ...
            const Operand* r = op->outputs[0];
            string input_name = string("v_") + sanitize_identifier(r->name);
...
int Graph::parse(const string& param)
{
...
Operator* Graph::new_operator(const string& type, const string& name)
{
...
Operator* Graph::new_operator_before(const string& type, const string& name, const Operator* cur)
{
    Operator* op = new Operator;
    op->type = type;
    op->name = name;
    ops.insert(find(ops.begin(), ops.end(), cur), op);
    return op;
}


Operator* Graph::new_operator_after(const string& type, const string& name, const Operator* cur)
{
    Operator* op = new Operator;
    op->type = type;
    op->name = name;
    ops.insert(find(ops.begin(), ops.end(), cur) + 1, op);
    return op;
}
...
Operand* Graph::new_operand(const torch::jit::Value* v)
{
    ...
    if (pt) {
        if (pt->scalarType().has_value() && pt->dim().has_value()) {
            ...
            for (int i = 0; i < ndim; i++) {
                if (pt->sizes()[i].has_value()) r->shape[i] = (int)pt->sizes()[i].value();
                else r->shape[i] = -1;
            }
        }
    }
...
Operand* Graph::new_operand(const string& name)
{
    Operand* r = new Operand;
    r->name = name;
    operands.push_back(r);
...
Operand* Graph::get_operand(const string& name)
{
...
const Operand* Graph::get_operand(const string& name) const
{
