...
int StoreZipReader::open(const string& path)
{
    close();
...
size_t StoreZipReader::get_file_size(const string& name)
{
...
int StoreZipReader::read_file(const string& name, char* data)
{
    if (filemetas.find(name) == filemetas.end()) {
        fprintf(stderr, "no such file %s\n", name.c_str());
        return -1;
...
int StoreZipWriter::open(const string& path)
{
    close();
...
int StoreZipWriter::write_file(const string& name, const char* data, size_t size)
{
    int offset = ftell(fp);
